...
news_emb_df = pd.read_csv(out_csv_path)
news_embeds = news_emb_df[[col for col in news_emb_df.columns if col.startswith('dim_')]].values

mean_news = news_embeds.mean(axis=0)
news_polar = to_polar(news_embeds, mean_news)

polar_out_path = os.path.join(out_dir, 'news_polar.png')
plot_polar(
    data1=None, 
    data2=news_polar,
    out_path=polar_out_path
)
print(f"🎨 Polar plot saved to {polar_out_path}")